...
def is_safe_dampened(report: list[int], dampened: bool) -> int | None:
    for i in range(1, len(report)):
        ...
        if not 1 <= diff <= 3:
            ...
            return is_safe_dampened(report=cut_forward_report, dampened=True) \
...
for report in reports:
    if is_safe_dampened(report=report, dampened=False) or is_safe_dampened(report=report[::-1], dampened=False):
        num_safe_dampened += 1
